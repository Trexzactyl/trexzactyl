...
@include('partials/admin.trexz.nav', ['activeTab' => 'approvals'])
...
@section('content')
    @yield('trexz::nav')
    <form action="{{ route('admin.trexz.approvals') }}" method="POST">
        <div class="row">
            <div class="col-xs-12">
                <div class="box
                    @if($enabled == 'true')
        ...
        <div class="col-xs-12">
            <div class="box box-success">
                <div class="box-header with-border">
                    ...
                    <form id="massdenyform" action="{{ route('admin.trexz.approvals.all', 'deny') }}" method="POST">
                        {!! csrf_field() !!}
                        <button id="denyAllBtn" class="btn btn-danger pull-right">Deny All</button>
                    </form>
                    <form id="massapproveform" action="{{ route('admin.trexz.approvals.all', 'approve') }}" method="POST">
                        {!! csrf_field() !!}
                            ...
                            @foreach ($users as $user)
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <form id="approveform" action="{{ route('admin.trexz.approvals.approve', $user->id) }}" method="POST">
                                            {!! csrf_field() !!}
                                    ...
                                    <td class="text-center">
                                        <form id="denyform" action="{{ route('admin.trexz.approvals.deny', $user->id) }}" method="POST">
                                            {!! csrf_field() !!}

...
@include('partials/admin.trexz.nav', ['activeTab' => 'index'])
...
@section('content-header')
    <h1>Trexz Settings<small>Configure Trexz-specific settings for the Panel.</small></h1>
    <ol class="breadcrumb">
        ...
        <div class="col-xs-12">
            ...
            ">
                <div class="box-header with-border">
                    <i class="fa fa-code-fork"></i> <h3 class="box-title">Software Release <small>Verify Trexz is up-to-date.</small></h3>
                </div>
                <div class="box-body">
                    @if ($version->isLatestPanel())
                        You are running Trexz <code>{{ config('app.version') }}</code>. 
                    @else
                        Trexz is not up-to-date. <code>{{ config('app.version') }} (current) -> <a href="https://github.com/trexz/trexz/releases/v{{ $version->getPanel() }}" target="_blank">{{ $version->getPanel() }}</a> (latest)</code>
                    @endif
                </div>
            </div>
        </div>

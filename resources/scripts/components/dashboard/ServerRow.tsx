...
const ServerCardContainer = styled.div<{ $bg?: string }>`
    ${tw`relative flex flex-col p-4 rounded-2xl border border-neutral-700 bg-neutral-900/40 backdrop-blur-xl transition-all duration-300 overflow-hidden`};
    ...
    &::before {
        ...
        position: absolute;
        inset: 0;
        ${({ $bg }) => $bg ? `background-image: url("${$bg}");` : 'background: linear-gradient(135deg, rgba(59, 130, 246, 0.05) 0%, rgba(147, 51, 234, 0.05) 100%);'}
...
const StatusIndicator = styled.div<{ $status: ServerPowerState | undefined }>`
    ${tw`flex items-center gap-1.5 px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest border transition-all duration-300`};
    ${({ $status }) =>
...
const ProgressBar = styled.div<{ $percent: number; $alarm?: boolean }>`
    ...
    & .fill {
        ...
        ${({ $alarm }) => ($alarm ? `background: linear-gradient(90deg, #ef4444, #f87171);` : `background: linear-gradient(90deg, #3b82f6, #60a5fa);`)};
        box-shadow: 0 0 8px ${({ $alarm }) => ($alarm ? 'rgba(239, 68, 68, 0.3)' : 'rgba(59, 130, 246, 0.3)')};
    }
`;

const Label = styled.div`
    ${tw`text-[10px] text-neutral-500 uppercase font-black tracking-widest flex items-center gap-1.5 mb-1`};
`;
            ...
            >
                <div css={tw`flex justify-between items-start mb-2`}>
                    <div css={tw`flex-1 min-w-0 pr-4`}>
                        <h4 css={tw`text-xl font-black text-white truncate group-hover:text-blue-400 transition-colors uppercase tracking-tight`}>{server.name}</h4>
                        <div css={tw`flex items-center gap-1.5 text-neutral-500 text-[10px] font-bold mt-1.5 uppercase`}>
                ...
                <div css={tw`flex-1 min-h-[60px]`}>
                    {!stats && !isSuspended && !server.isTransferring && server.status !== 'installing' ? (
                        ...
                        <ResourceSection>
                            <div>
                                <Label><Icon.Cpu size={10} className="text-blue-400" /> CPU</Label>
                            ...
                            <div>
                                <Label><Icon.Activity size={10} className="text-purple-400" /> RAM</Label>
                            ...
                            <div>
                                <Label><Icon.Database size={10} className="text-cyan-400" /> DISK</Label>
                                <Value>
                                    {bytesToString(stats?.diskUsageInBytes || 0)}
                                </Value>
                                <ProgressBar $percent={stats ? Math.min((stats.diskUsageInBytes / (server.limits.disk * 1024 * 1024 || 1)) * 100, 100) : 0}>
                                    <div className="fill" />
                                </ProgressBar>
                            </div>
                        </ResourceSection>
                ...
                <div css={tw`mt-4 pt-3 border-t border-neutral-800/50 flex justify-between items-center text-[9px] text-neutral-500 font-bold uppercase tracking-widest`}>
                    <div css={tw`flex items-center gap-1.5`}>
                        <Icon.Box size={10} className="text-neutral-600" />
                        {server.node}
                    </div>
                    <div css={tw`flex items-center gap-1.5 px-2 py-1 rounded bg-neutral-800/30 group-hover:bg-blue-500/10 group-hover:text-blue-400 transition-colors cursor-pointer`}>

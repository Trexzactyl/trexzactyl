...
import { faCopy, faMobileAlt, faCheckCircle, faMoneyBillWave } from '@fortawesome/free-solid-svg-icons';
...
const GatewayCard = styled(motion.div) <{ active: boolean; color: string }>`
    ...
    ${(props) => props.active ? `border-color: ${props.color}; background: rgba(0,0,0,0.3);` : tw`border-neutral-700 bg-neutral-900 opacity-60 hover:opacity-100`};
...
const PriceDisplay = ({ amount, currencyCode, price, color }: { amount: string, currencyCode: string, price: number, color: string }) => {
            ...
            {({ isSubmitting, values, setFieldValue }) => {
                ...
                return (
                    <Form>
                            ...
                            {gateways.bkash && (
                                ...
                                >
                                    <div css={tw`flex items-center justify-between`}>
                                        ...
                                        {values.currency === 'bkash' && <FontAwesomeIcon icon={faCheckCircle as any} css={tw`text-[#E2136E] text-2xl`} />}
                                    </div>
                            ...
                            {gateways.nagad && (
                                ...
                                >
                                    <div css={tw`flex items-center justify-between`}>
                                        ...
                                        {values.currency === 'nagad' && <FontAwesomeIcon icon={faCheckCircle as any} css={tw`text-[#F6921E] text-2xl`} />}
                                    </div>
                        ...
                        <PriceDisplay amount={values.amount} currencyCode={currency} price={creditPrice} color={activeColor} />
                        ...
                        <AnimatePresence exitBeforeEnter>
                            ...
                            >
                                <h3 css={tw`text-xl font-bold text-white mb-6 flex items-center`}>
                                    <FontAwesomeIcon icon={faMobileAlt as any} css={tw`mr-3 opacity-80`} />
                                ...
                                <div css={tw`space-y-4`}>
                                    <Step>
                                        ...
                                        <div css={tw`text-gray-300`}>
                                            Open your <strong>{activeName} App</strong> and select <strong>Send Money</strong>.
                                        </div>
                                    </Step>
                                    <Step>
                                        <StepNumber color={activeColor}>2</StepNumber>
                                        <div css={tw`text-gray-300`}>
                                            Enter Receiver Number: <span css={tw`text-white font-mono font-bold text-lg ml-1`}>{activeNumber}</span>
                                            <CopyButton type="button" onClick={() => copyToClipboard(activeNumber || '')}>
                                                {copied ? <FontAwesomeIcon icon={faCheckCircle as any} /> : <FontAwesomeIcon icon={faCopy as any} />} {copied ? 'COPIED' : 'COPY'}
                                            </CopyButton>
                                        </div>
                                    </Step>
                                    <Step>
                                        ...
                                        <div css={tw`text-gray-300`}>
                                            Enter Amount: <strong style={{ color: activeColor }}>{((parseInt(values.amount) || 0) / 100 * creditPrice).toFixed(2)} {currency}</strong>
                                        </div>
                                    </Step>
                                    <Step>
                                    ...
                                    <div tw="mt-4 p-4 rounded-lg bg-black/40 border border-white/5">
                                        ...
                                        <div tw="flex justify-between items-center">
                                            ...
                                            <span tw="text-white font-bold text-lg">
                                                {(Number(values.amount) * (storefront?.gateways.conversion_rate || 115)).toFixed(2)} BDT
                                            </span>
                                        </div>
                        ...
                        <div css={tw`mt-8 p-6 bg-neutral-800 rounded-xl border border-neutral-700`}>
                            ...
                            <div css={tw`mt-6 text-right`}>
                                <Button type={'submit'} disabled={isSubmitting} size={Button.Sizes.Large} style={{ background: activeColor, borderColor: activeColor }}>

...
export default ({ setOpen }: { setOpen: Dispatch<SetStateAction<BillingSetupDialog>> }) => {
    ...
    return (
        <Dialog open onClose={() => setOpen('none')} title={'How to configure PayPal'}>
            ...
            <p className={'text-lg font-semibold mt-4 text-white'}>3. Check Trexz for PayPal method</p>
            Finally, go back to Trexz and visit the billing store. Select a plan, and head to the checkout. You
            should now be able to checkout with PayPal via Trexz.

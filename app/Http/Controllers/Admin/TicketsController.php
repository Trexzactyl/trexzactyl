...
use Trexz\Http\Requests\Admin\Tickets\TicketStatusRequest;
use Trexz\Http\Requests\Admin\Tickets\TicketToggleRequest;
use Trexz\Contracts\Repository\SettingsRepositoryInterface;
use Trexz\Http\Requests\Admin\Tickets\TicketMessageRequest;
...
class TicketsController extends Controller
{
    ...
    public function index(): View
    {
        return $this->view->make('admin.tickets.index', [
            'tickets' => Ticket::all(),
            'enabled' => $this->settings->get('trexz::tickets:enabled', false),
            'max' => $this->settings->get('trexz::tickets:max', 3),
        ]);
    }
    ...
    public function toggle(TicketToggleRequest $request): RedirectResponse
    {
        foreach ($request->normalize() as $key => $value) {
            $this->settings->set('trexz::tickets:' . $key, $value);
        }
